function spaceweather_forecast(file_path::String)
...
function get_spaceweather_forecast(date::Date)
    row = spaceweather_forecast_data[][spaceweather_forecast_data[].Date .== date, :]
    f107 = row.F107[1]
    ap = row.AP[1]
    return f107, f107, ap
end
